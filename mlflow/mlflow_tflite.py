...
from mlflow.utils.environment import _CONDA_ENV_FILE_NAME, _CONSTRAINTS_FILE_NAME, _REQUIREMENTS_FILE_NAME, _process_conda_env, _process_pip_requirements
from mlflow.utils.file_utils import write_to
from mlflow.utils.model_utils import _get_flavor_configuration, _validate_and_prepare_target_save_path
...
def log_model(tflite_model, artifact_path, conda_env=None, registered_model_name=None, **kwargs):
   Model.log(artifact_path, mlflow_tflite, registered_model_name, tflite_model=tflite_model, **kwargs)


def save_model(tflite_model, path, mlflow_model=None, conda_env=None, pip_requirements=None, extra_pip_requirements=None):
   _validate_and_prepare_target_save_path(path)

   if mlflow_model == None:
      mlflow_model = Model()
...
def load_model(model_uri, dst_path=None):
   ...
   with open(tflite_model_file_path, "rb") as f:
      model = f.read()
